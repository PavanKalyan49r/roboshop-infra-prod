resource "aws_cloudfront_distribution" "pavanaws" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.pavanaws.domain_name
        zone_id = aws_cloudfront_distribution.pavanaws.hosted_zone_id
      }
    }
  ]
}